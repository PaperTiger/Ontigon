...
export default function RoadmapPage() {
  return (
    <div>
      <section style={{ padding: 'clamp(130px,20vh,220px) clamp(20px,5vw,64px) clamp(56px,8vh,100px)', maxWidth: 1320, margin: '0 auto' }}>
      ...
      <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 clamp(20px,5vw,64px) clamp(80px,12vh,160px)' }}>
        <div style={{ position: 'relative' }}>
          ...
          {/* Complete */}
          <Reveal style={{ padding: 'clamp(44px,6vh,70px) 0 clamp(44px,6vh,70px) 60px', borderBottom: `1px solid ${C.hair}`, position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 1, background: 'rgba(26,25,22,.1)', border: '1px solid rgba(26,25,22,.1)', maxWidth: 820 }}>
              {[
                ...
                <div key={i} style={{ background: '#f1f1f2', padding: '20px 22px' }}>
                  <div style={{ fontFamily: mono, fontSize: 11, letterSpacing: '.06em', color: C.mute, marginBottom: 6 }}>{tag}</div>
                  <div style={{ fontSize: 15, lineHeight: 1.5, color: C.ink }}>{text}</div>
                </div>
              ))}
            </div>
          </Reveal>
          ...
          <Reveal style={{ padding: 'clamp(44px,6vh,70px) 0 clamp(44px,6vh,70px) 60px', borderBottom: `1px solid ${C.hair}`, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', borderTop: '1px solid rgba(26,25,22,.1)' }}>
              {[
                ...
                <div key={label} style={{ padding: '18px 0', borderBottom: '1px solid rgba(26,25,22,.1)', display: 'grid', gridTemplateColumns: '2fr 3fr', gap: 24, alignItems: 'start' }}>
                  <span style={{ fontFamily: mono, fontSize: 12, letterSpacing: '.04em', color: C.mute }}>{label}</span>
                  <span style={{ fontSize: 15, lineHeight: 1.6, color: C.body }}>{text}</span>
                </div>
              ))}
            </div>
          </Reveal>
          ...
          {/* Horizon */}
          <Reveal style={{ padding: 'clamp(44px,6vh,70px) 0 0 60px', position: 'relative' }}>
